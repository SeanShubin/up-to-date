...
import com.seanshubin.devon.core.devon.{DevonMarshaller, DevonNull}
...
class ConfigurationValidatorTest extends FunSuite with EasyMockSugar {
  ...
  test("at least one command line argument required") {
    val configurationValidator: ConfigurationValidator = new ConfigurationValidatorImpl(dummyFileSystem, dummyJsonMarshaller, dummyDevonMarshaller)
  ...
  test("configuration file must exist") {
    ...
    val configurationValidator: ConfigurationValidator = new ConfigurationValidatorImpl(fileSystem, dummyJsonMarshaller, dummyDevonMarshaller)
  ...
  test("json must be in the correct shape") {
    ...
    val devonMarshaller: DevonMarshaller = mock[DevonMarshaller]
    val configurationValidator: ConfigurationValidator = new ConfigurationValidatorImpl(fileSystem, jsonMarshaller, devonMarshaller)
    ...
    whenExecuting(fileSystem, jsonMarshaller, devonMarshaller) {
      val actual = configurationValidator.validate(Seq("file name"))
  ...
  test("well formed json does not meet validation rules") {
    ...
    expecting {
      ...
      devonMarshaller.fromValue(parsedFromJson).andReturn(DevonNull)
      devonMarshaller.toPretty(DevonNull).andReturn(Nil)
    }
